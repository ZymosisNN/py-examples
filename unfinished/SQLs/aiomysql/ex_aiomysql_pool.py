...
class AsyncDBDataFetcher(AbstractVCCDataFetcher):
    def __init__(self, param: DBParam, *,
        ...
        self.conn_pools: dict[str, aiomysql.Pool] = {}
    ...
    async def execute_query(self, query: str, host='', db_name=''):
        results = await self.execute_queries([query], host, db_name)
        return results[0]

    async def execute_queries(self, queries: Iterable[str], host='', db_name=''):
        host = host or self.param.host
        db_name = db_name or self.param.db_name
        pool = await self.get_conn_pool(host)
    ...
    async def get_conn_pool(self, host: str):
        async with self.lock:
            if host not in self.conn_pools:
                self.log.info(f'Creating pool for: "{host}"')
                self.conn_pools[host] = await aiomysql.create_pool(
                    host=host,
                    port=self.param.port,
                    user=self.param.username,
                    password=self.param.password,
                    minsize=1, maxsize=20, echo=False
                )

        return self.conn_pools[host]
